...
export function ChatBot() {
  ...
  const [messages, setMessages] = useState<Array<{ text: string; isBot: boolean }>>([
    {
      text: "Bonjour! Je suis l'assistant virtuel de jrdriving. Comment puis-je vous aider aujourd'hui?",
      isBot: true,
    },
  ]);
  ...
  const handleSend = () => {
    ...
    setTimeout(() => {
      ...
      if (lowerInput.includes('devis') || lowerInput.includes('tarif') || lowerInput.includes('prix')) {
        response =
          "Pour obtenir un devis personnalisé, je vous invite à remplir notre formulaire en ligne. Cela ne prend que quelques minutes et vous recevrez une estimation rapidement!";
      } else if (lowerInput.includes('mission') || lowerInput.includes('suivi')) {
        response =
          "Vous pouvez suivre votre mission en temps réel depuis votre espace client. Si vous n'avez pas encore de compte, je peux vous aider à en créer un!";
      } else if (lowerInput.includes('service') || lowerInput.includes('convoyage')) {
        response =
          'Nous proposons du convoyage automobile B2B (loueurs, garages, concessions) et B2C (particuliers). Nous assurons des transports express et multi-étapes partout en France!';
      } else if (
        ...
        response =
          'Vous pouvez nous contacter par email à contact@jrdriving.galaxjr.digital ou par téléphone au +33 1 23 45 67 89. Notre équipe est disponible du lundi au vendredi de 8h à 18h.';
      } else if (lowerInput.includes('chauffeur') || lowerInput.includes('devenir')) {
        response =
          'Vous souhaitez rejoindre notre équipe de chauffeurs? Excellent! Contactez-nous pour connaître les conditions et les démarches à suivre.';
      } else {
        response =
          "Je suis là pour vous aider! Vous pouvez me poser des questions sur nos services, demander un devis, suivre une mission ou nous contacter. Comment puis-je vous assister?";
      }
        ...
        <div className="fixed bottom-6 right-6 w-96 h-[32rem] bg-white rounded-2xl shadow-2xl flex flex-col z-50">
          ...
          <div className="flex-1 overflow-y-auto p-4 space-y-4">
            {messages.map((message, index) => (
              <div key={index} className={message.isBot ? 'flex justify-start' : 'flex justify-end'}>
                <div
                  className={
